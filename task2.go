...
指针
	...
	考察点 ：指针运算、切片操作。
*/
func recePointerAdd(num *int) {
	*num += 10
}
func recePointerMul(num []*int) {
	for i := range num {
		*num[i] *= 2
	}
}
...
func (e *Employee) PrintInfo() {
	fmt.Println("name", e.p.Name, " age", e.p.Age, " EmployeeID", e.EmployeeID)
}
...
Channel
	...
	考察点 ：通道的缓冲机制。
*/
func sendNum(ch chan<- int, num int) {
	for i := 1; i <= num; i++ {
		ch <- i
	}
}
func receNum(ch <-chan int, num int) {
	for i := 1; i <= num; i++ {
		fmt.Println(<-ch)
	}
}
...
func (a *atomicCounter) atomicInc1000(wg *sync.WaitGroup) {
	defer wg.Done()
	for i := 0; i < 1000; i++ {
